...
class InstallmentsController extends Controller
{
    ...
    public function payByAlipay(Installment $installment) {
        if ($installment->order->closed) {
            throw new InvalidRequestException('對應的商品訂單已被關閉');
        }
        if ($installment->status === Installment::STATUS_FINISHED) {
            throw new InvalidRequestException('該分期訂單已結清');
        }
    ...
    public function payByPaypal(Installment $installment) {
        ...
        // 调用支付宝的网页支付
        $params = array(
            'cancelUrl' => route('installments.paypal.return'),
            'returnUrl' => route('installments.paypal.return'), // in your case             //  you have registered in the routes 'payment_success'
            'amount' => $nextItem->total,
    ...
    public function paypalReturn(){
        ...
        if(isset($paypalResponse['PAYMENTINFO_0_ACK']) && $paypalResponse['PAYMENTINFO_0_ACK'] === 'Success') {
            ...
            // 使用事务，保证数据一致性
            \DB::transaction(function () use ($no, $installment, $item) {
                // 更新对应的还款计划
                $item->update([
                    'paid_at'        => Carbon::now(), // 支付时间
                    'payment_method' => 'paypal', // 支付方式
                    'payment_no'     => $installment->no, // 订单号
                ]);
