                ...
                <div class="panel-body product-info">
                            ...
                            @if($product->type === \App\Models\Product::TYPE_CROWDFUNDING)
                                <div class="crowdfunding-info">
                                    ...
                                    <div class="progress-info">
                                        <span class="current-progress">當前進度：{{ $product->crowdfunding->percent }}%</span>
                                    ...
                                    @if ($product->crowdfunding->status === \App\Models\CrowdfundingProduct::STATUS_FUNDING)
                                        <div>此項目必须在
                                            ...
                                            募資將在<span class="text-red">{{ $product->crowdfunding->end_at->diffForHumans(now()) }}</span>结束！
                                        </div>
                                    @endif
                                </div>
                        @else
                            <!-- 原普通商品模块开始 -->
                                ...
                                <div class="sales_and_reviews">
                                    ...
                                    <div class="rating" title="评分 {{ $product->rating }}">評分
                        ...
                        <!-- 众筹商品模块结束 -->
                            ...
                            <!-- 众筹商品下单按钮开始 -->
                                @if($product->type === \App\Models\Product::TYPE_CROWDFUNDING)
                                    @if(Auth::check())
                                        @if($product->crowdfunding->status === \App\Models\CrowdfundingProduct::STATUS_FUNDING)
                                            <button class="btn btn-primary btn-crowdfunding">參加募資</button>
                                        @else
                                        ...
                                        @endif
                                    @else
                                        <a class="btn btn-primary" href="{{ route('login') }}">請先登入</a>
                                    @endif
                                @else
                    ...
                    <div class="product-detail">
                        <ul class="nav nav-tabs" role="tablist">
                            ...
                            <li role="presentation"><a href="#product-reviews-tab" aria-controls="product-reviews-tab" role="tab" data-toggle="tab">用户評價</a></li>
                        </ul>
        ...
        $(document).ready(function () {
            ...
            $('.btn-crowdfunding').click(function () {
                ...
                $form.append('<div class="form-group">' +
                    '<label class="control-label col-sm-3">選擇地址</label>' +
                    '<div class="col-sm-9">' +
                ...
                $form.append('<div class="form-group">' +
                    '<label class="control-label col-sm-3">購買數量</label>' +
